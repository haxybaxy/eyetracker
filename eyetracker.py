...
class EyeTrackingHeatmap:
    def __init__(self, stimulus_path=None, camera_id=0, heatmap_resolution=(800, 600)):
        ...
        if stimulus_path and os.path.exists(stimulus_path):
            ...
            self.stimulus = np.ones((heatmap_resolution[1], heatmap_resolution[0], 3), dtype=np.uint8) * 255
        ...
        self.screen_corners = np.array([[0, 0], [heatmap_resolution[0], 0], 
                                        [0, heatmap_resolution[1]], [heatmap_resolution[0], heatmap_resolution[1]]])
        self.eye_corners = []
    ...
    def start_camera(self):
        ...
        if not self.cap.isOpened():
            raise Exception("Could not open camera. Check if camera is connected and ID is correct.")
    ...
    def calibrate(self):
        ...
        for i, corner in enumerate(self.screen_corners):
            ...
            cv2.putText(calibration_display, f"Look at the {corners_text[i]} corner", 
                ...
                for (x, y, w, h) in faces:
                    ...
                    if len(eyes) >= 2:
                        ...
                        for (ex, ey, ew, eh) in eyes[:2]:  # Use first two detected eyes
                            ...
                            cv2.circle(frame, (eye_center[0], eye_center[1]), 5, (255, 0, 0), -1)
                        
                        # Average position of the two eyes
                        eye_pos = np.mean(eye_centers, axis=0)
                        cv2.circle(frame, (int(eye_pos[0]), int(eye_pos[1])), 10, (0, 255, 0), -1)
    ...
    def track_eyes(self, duration=10):
        ...
        while self.running and (time.time() - start_time < duration):
            ...
            for (x, y, w, h) in faces:
                ...
                if len(eyes) >= 2:
                    ...
                    for (ex, ey, ew, eh) in eyes[:2]:
                        eye_center = [x + ex + ew//2, y + ey + eh//2]
                        eye_centers.append(eye_center)
                        cv2.circle(display_frame, (eye_center[0], eye_center[1]), 5, (0, 0, 255), -1)
                    
                    # Average position of the two eyes
                    eye_pos = np.mean(eye_centers, axis=0)
                    cv2.circle(display_frame, (int(eye_pos[0]), int(eye_pos[1])), 10, (0, 255, 0), -1)
                    
                    # Map eye position to screen coordinates
                    screen_pos = self.map_eye_to_screen(eye_pos)
                    if screen_pos:
                        # Add to gaze points if within bounds
                        if (0 <= screen_pos[0] < self.heatmap_resolution[0] and 
                            0 <= screen_pos[1] < self.heatmap_resolution[1]):
            ...
            cv2.putText(display_frame, f"Time remaining: {remaining}s", (10, 30), 
...
def main():
    ...
    stimulus_path = input("Enter path to stimulus image (leave empty to use blank screen): ").strip()
    ...
    try:
        ...
        print("First, let's calibrate by looking at each corner of the screen when prompted.")
        if tracker.calibrate():
            duration = int(input("Enter tracking duration in seconds: ") or "10")
            if tracker.track_eyes(duration):
                save_path = input("Enter path to save heatmap (or press Enter to skip saving): ").strip()
